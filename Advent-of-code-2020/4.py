...
for x in l:
    if x!="\n":
        ...
        if "byr" in a:
            ...
            j=a.find("byr")
            an_nastere+=a[j+4]
            an_nastere = an_nastere + a[j + 5]
            an_nastere = an_nastere + a[j + 6]
            an_nastere = an_nastere + a[j + 7]
            if int(an_nastere)<=2002 and int(an_nastere)>=1920:
                byr += 1
            else:
                byr=0
        ...
        if "iyr" in a:
            ...
            j = a.find("iyr")
            issue_year += a[j + 4]
            issue_year = issue_year + a[j + 5]
            issue_year = issue_year + a[j + 6]
            issue_year = issue_year + a[j + 7]
            if int(issue_year) <= 2020 and int(issue_year) >= 2010:
                iyr += 1
            else:
                iyr = 0
        if "eyr" in a:
            ...
            j = a.find("eyr")
            exp_year += a[j + 4]
            exp_year = exp_year + a[j + 5]
            exp_year = exp_year + a[j + 6]
            exp_year = exp_year + a[j + 7]
            if int(exp_year) <= 2030 and int(exp_year) >= 2020:
                eyr += 1
            else:
                eyr = 0
        if "hgt" in a:
            ...
            if a[j+6]=="i":
                ok=1
                if int(height)<=76 and int(height)>=59:
                    hgt+=1
            elif a[j+6]=="c":
                continue
            elif a[j+6] in "1234567890" and a[j+7]=="i" :
                continue
            else:
                if a[j+6] in "1234567890" and a[j+7]=="c":
                    height+=a[j+6]
                    if int(height)<=193 and int(height)>=150:
                        hgt+=1
        if "hcl" in a:
            j=a.find("hcl")
            if a[j+4]=="#":
        ...
        if "ecl" in a:
            ...
            eye_c += a[j + 5]
            eye_c += a[j + 6]
            if eye_c=="amb" or eye_c=="blu" or eye_c=="brn" or eye_c=="gry" or eye_c=="grn" or eye_c=="hzl" or eye_c=="oth":
                ecl += 1
        if "pid" in a:
            j=a.find("pid")
            ok=1
            for i in range(4,13):
                if a[j+i] not in "1234567890":
                    ok=0
                    break
            if ok:
                pid += 1
